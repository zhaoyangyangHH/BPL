...
class DQN(nn.Module):
    ...
    def singleBatch(self, raw_batch, params):
        ...
        if self.use_icm:
            ...
            # print 'icm_loss', icm_loss #icm_loss de mean
            # print 'r_i', i_r.mean().cpu().numpy() #icm_loss de sum, with the loss bigger, the reward increase? = agent_dqn intrinsic_reward
    ...
    def get_intrinsic_reward(self, state, next_state, action):
        ...
        #icm_loss tensor([[8.8015e-01, 1.5371e-02, 4.3113e-02, 4.8338e-03, 4.0772e-02, 4.0767e-04,....]], grad_fn=<MseLossBackward>)
        i_r = icm_loss.sum(-1).detach()
        # print 'i_r', i_r
        #sum above number
        i_r = (i_r - i_r.mean()) / (i_r.std() + 1e-10)
    ...
    def get_icm_loss(self, state, next_state, action):
        # print ('state_2', state)
        # print ('state_next_2', next_state)
        # print ('action_2', action)
        ...
        # print ('state_3', state)
        # print ('state_next_3', next_state)
        # print ('action_3', action)
        icm_loss = F.mse_loss(state_pred, next_state.detach(), reduce=False)
        # icm_loss = icm_loss.mean()
        # print 'icm_loss', icm_loss
        # print 'icm_loss', icm_loss.detach().numpy()
        # return icm_loss.detach().numpy()
