...
class RuleTaxiSimulator(UserSimulator):
    """ A rule-based user simulator for testing dialog policy """
    
    def __init__(self, taxi_dict=None, act_set=None, slot_set=None, start_set=None, params=None):
        """ Constructor shared by all user simulators """
        
        self.taxi_dict = taxi_dict
    ...
    def initialize_episode(self, selected_goal=None):
        ...
        self.goal['request_slots']['taxi'] = 'UNK'
    ...
    def _sample_action(self):
        ...
        if len(self.goal['inform_slots']) > 0:
            ...
            if 'pickup_location' in self.goal['inform_slots'].keys(): # 'taxiname' must appear in the first user turn
                self.state['inform_slots']['pickup_location'] = self.goal['inform_slots']['pickup_location']
            ...
            for slot in self.goal['inform_slots'].keys():
                if known_slot == slot or slot == 'pickup_location' or slot == 'greeting': continue
        ...
        for slot in user_action['inform_slots'].keys():
            ...
            if slot_err_prob_sample < self.slot_err_probability: # add noise for slot level
                if self.slot_err_mode == 0: # replace the slot_value only
                    if slot in self.taxi_dict.keys(): user_action['inform_slots'][slot] = random.choice(self.taxi_dict[slot])
                    ...
                    if slot_err_random <= 0.33:
                        ...
                        random_slot = random.choice(self.taxi_dict.keys())
                        user_action[random_slot] = random.choice(self.taxi_dict[random_slot])
                    else:
                        del user_action['inform_slots'][slot]
    ...
    def debug_falk_goal(self):
        ...
        self.goal['inform_slots']['numberofpeople'] = '10'
        ...
        self.goal['inform_slots']['pickup_location'] = 'TownePlace Suites'
        self.goal['inform_slots']['pickup_location_city'] = 'wilmington'
        self.goal['inform_slots']['state'] = 'nc'
        
        self.goal['request_slots'].clear()
        self.goal['request_slots']['taxi'] = 'UNK'
        self.goal['request_slots']['car_type'] = 'UNK'
        self.goal['request_slots']['pickup_time'] = 'UNK'
        self.goal['request_slots']['dropoff_location'] = 'UNK'
        self.goal['request_slots']['cost'] = 'UNK'
    ...
    def response_thanks(self, system_action):
        ...
        if 'taxi' in request_slot_set:
            request_slot_set.remove('taxi')
        rest_slot_set = copy.deepcopy(self.state['rest_slots'])
        if 'taxi' in rest_slot_set:
            rest_slot_set.remove('taxi')
        ...
        if 'taxi' in system_action['inform_slots'].keys():
            if system_action['inform_slots']['taxi'] == dialog_config.NO_VALUE_MATCH:
                self.dialog_status = dialog_config.FAILED_DIALOG
    ...
    def response_inform(self, system_action):
        ...
        if 'taskcomplete' in system_action['inform_slots'].keys(): # check all the constraints from agents with user goal
            ...
            if system_action['inform_slots']['taskcomplete'] == dialog_config.NO_VALUE_MATCH:
                self.state['history_slots']['taxi'] = dialog_config.NO_VALUE_MATCH
                if 'taxi' in self.state['rest_slots']: self.state['rest_slots'].remove('taxi')
                if 'taxi' in self.state['request_slots'].keys(): del self.state['request_slots']['taxi']
            ...
            for slot in system_action['inform_slots'].keys():
                ...
                if slot in self.goal['inform_slots'].keys():
                    ...
                    if len(self.state['request_slots']) > 0:
                        ...
                        if 'taxi' in request_set:
                            request_set.remove('taxi')
